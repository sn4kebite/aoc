...
for line in sys.stdin:
    ...
    if m:
        value, bot_n = m.groups()
        bot = get_bot(int(bot_n))
        bot.add(value)
        continue
    ...
    if m:
        ...
        from_bot = get_bot(from_bot_n)
        for w, ww, n in ((which_1, to_which_1, int(to_n_1)), (which_2, to_which_1, int(to_n_2))):
            if ww == 'bot':
                out = get_bot(n)
            elif ww == 'output':
                out = get_output(n)
...
print('Done processing after {} iterations'.format(i))
