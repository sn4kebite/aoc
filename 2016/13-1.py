...
class State(object):
    ...
    def get_moves(self, dx, dy):
        ...
        if not is_wall(self.x, self.y+1):
            moves.append(State(self.x, self.y+1, self.step+1))
        moves.sort(key = lambda state: abs(dx - state.x) + abs(dy - state.y))
...
while len(queue):
    state = queue.popleft()
    visited.add(state)
    if state.x == 31 and state.y == 39:
        print('Done after {} steps'.format(state.step))
        break
    queue.extend(state.get_moves(31, 39))
